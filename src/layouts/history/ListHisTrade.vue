  ...
  methods: {
    ...
    reloadList() {
      ...
      if(this.startDate && this.endDate){
        ...
        this.getListAllTrade(queryString)
      }else if(!this.startDate && !this.endDate && this.valueType ){
        let filler = {
          type_key: this.valueType
        }
        let queryString = Object.keys(filler).map(key => key + '=' + filler[key]).join('&');

        this.getListAllTrade(queryString)
      }
      else if(!this.startDate  && !this.endDate && !this.valueType){
        this.getListAllTrade(query)
      }


    },
    getListAllTrade(query){
      AuthenticationService.getTradeListHistory(query).then((resp) => {
        this.$vs.loading.close("#loading-corners > .con-vs-loading");
