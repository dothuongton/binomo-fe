...
<template>
  <div id="list-history-withdrawal" class="data-list-container">
    ...
    <div id="loading-corners" class="vs-con-loading__container">
      ...
      <vs-table ref="table" multiple v-model="selectedUser" pagination :max-items="itemsPerPage" search
                :data="products">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <!-- ACTION - DROPDOWN -->
            <vs-dropdown vs-trigger-click class="dd-actions cursor-pointer mr-4 mt-4">
            ...
            <div class="flex mb-4 mt-3">
              <div class="flex mt-5">
                ...
                <label for="endDatePicker" class="mt-2">Đến ngày:</label>
                <datepicker
              ...
              <div class="mt-2 mt-5">
                <label class="mr-3" for="typeSelect">Chọn Type:</label>
                <select class="type-select" id="typeSelect" v-model="valueType">
                  <option class="mt-2" v-for="(item, i) in dataValueType" :key="i" :value="item.value">{{
                      item.text
                    }}
                  </option>
                </select>
        ...
        <template slot="thead">
          ...
          <vs-th sort-key="note">Tên ngân hàng</vs-th>
          <vs-th sort-key="note">Chi nhánh</vs-th>
          <vs-th sort-key="note">Chủ tài khoản</vs-th>
          <vs-th sort-key="note">Số tài khoản</vs-th>
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="de-name font-medium truncate">
                    <span>
                      <IconCrypto style="width: 20px" :coinname="getIconType(tr.currency)" color="color" format="svg"
                                  v-if="tr.paypal == null"/>
            ...
            <vs-td>
              <vs-chip :color="getOrderStatusColor(tr.status)" class="de-status">
                {{ getOrderStatusColorText(tr.status) | title }}
              </vs-chip>
            </vs-td>
            ...
            <vs-td class="whitespace-no-wrap text-center">
              <vx-tooltip v-if="tr.currency == 'vnd' && tr.status === 0" style="float: left" color="primary"
                          text="Xử lý">
                <vs-button color="dark" type="line" icon-pack="feather" icon="icon-feather"
                           @click.stop="toggleDataSidebarHandleMoney(tr)"></vs-button>
              </vx-tooltip>
              <template v-else>
                <vx-tooltip v-if="tr.status == 0 && tr.network == 'bep20' || tr.status == 0 && tr.network == 'erc20' "
                            style="float: left" color="warning" text="Duyệt">
                  <vs-button :disabled="clickDone" color="dark" type="line" icon-pack="feather" icon="icon-check"
                             @click.stop="doneWithdra(tr.id, indextr, 1, tr.to_u, tr.amount, tr.network)">Duyệt
                  </vs-button>
                </vx-tooltip>
                <vx-tooltip v-if="tr.status == 0 && tr.network == 'bep20' || tr.status == 0 && tr.network == 'erc20' "
                            style="float: left" color="danger" text="Từ chối">
                  <vs-button :disabled="clickRefuseDone" color="red" type="line" icon-pack="feather" icon="icon-check"
                             @click.stop="doneRefuseWithdra(tr.id, indextr, tr.amount, tr.email, tr.fee_withdraw)">Từ
                    chối
                  </vs-button>
                </vx-tooltip>
...
import Vue from 'vue'
import TransactionCheckPopup from "@/layouts/history/TransactionCheckPopup.vue";
...
`;
    ...
    getOrderStatusColorText(status) {
      ...
      if (status == -1 || status == 2) return "Từ chối"
      return "Đang xử lý"
    },
    ...
    reloadList() {
      ...
      this.$store.dispatch('setToken', token)
      let obj = {
        startDate: this.formatDatePayload(this.startDate),
        endDate: this.formatDatePayload(this.endDate),
        status: this.valueType
      }
      if (this.startDate && this.endDate) {
        if (this.startDate > this.endDate) {
          return this.$vs.notify({
            title: 'Thời gian tra soát không hợp lệ',
            color: 'danger',
            iconPack: 'feather',
            icon: 'icon-heart'
          });
          ...
          this.getDataHisWithdraw(queryString)
        }
      } else if (!this.startDate && !this.endDate && this.valueType || this.valueType === 0) {
        let filter = {
          status: this.valueType
        }
        let query = Object.keys(filter).map(key => key + '=' + filter[key]).join('&');
        this.getDataHisWithdraw(query)
      } else if (!this.startDate && !this.endDate && !this.valueType) {
        this.getDataHisWithdraw(null)
      }

    },
    getDataHisWithdraw(query) {
      AuthenticationService.getWithdrawalListHistory(query)
        .then((resp) => {
