...
<template>
  <div id="list-bet-trade" class="data-list-container">
    ...
    <div class="mb-2" v-if="dataGetAmountBet[0]">
      <div class="font-bold">
        *** Số tiền đăt cược:{{formatPrice(dataGetAmountBet[0].amount_bet)}} $
      </div>
      <div class="text-success font-bold">
        *** Tổng thắng:{{formatPrice(dataGetAmountBet[0].amount_win)}} $
      </div>
      <div class="text-danger font-bold">
        *** Tổng thua:{{formatPrice(dataGetAmountBet[0].amount_lose)}} $
      </div>
    </div>
    <div id="loading-corners" class="vs-con-loading__container">

      <vs-table ref="table" multiple v-model="selectedUser" pagination :max-items="itemsPerPage" search :data="productsFake">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <div class="flex items-center mb-8">
            <div class="flex mt-5">
              <label for="startDatePicker" class="mt-2 mr-2">Từ ngày:</label>
              <datepicker
                id="startDatePicker"
                class="mr-5 focus:border-opacity-50"
                selected
                v-model="startDate"
              ></datepicker>
              <span class="flex items-center">-</span>
              <label for="endDatePicker" class="flex items-center ml-2 border" >Đến ngày:</label>
              <datepicker

                class="ml-5 mr-10 focus:border-opacity-50"
                id="endDatePicker"
                v-model="endDate"
              ></datepicker>
              <label for="emailSearch" class="mr-2 flex items-center">Email</label>
              <input type="text" class="border rounded" v-model="emailSearch">
            </div>
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ productsFake.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : productsFake.length }} of {{ queriedItems }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
            ...
            <vs-dropdown-menu>
              ...
              <vs-dropdown-item @click="itemsPerPage=11">
                <span>10</span>
              </vs-dropdown-item>
              <vs-dropdown-item @click="itemsPerPage=16">
                <span>15</span>
              </vs-dropdown-item>
          ...
          <template slot-scope="{data}">
            <tbody>
                <vs-tr v-if = "data.amount_win === null">
                  No data
                </vs-tr>
                <vs-tr v-else :data="tr" :key="indextr" v-for="(tr, indextr) in data.filter((e) => e.type_account)">
...
<!--                <vs-tr  :data="tr" :key="indextr" v-for="(tr, indextr) in data">-->
                  ...
                  <vs-td>
                    ...
                    <p>Phiên: {{ convertSession(tr.created_at) }}</p>
                  </vs-td>

                  <vs-td>
                    <p class="de-create">{{convertSessionTimeOpen(tr.created_at)  }}</p>
                  </vs-td>
...
export default {
  ...
  methods: {
    ...
    convertSessionTimeOpen(value){
      if (value) {
        return moment(value).subtract(1, 'minutes').format('YYYY-MM-DD HH:mm:ss');
      }
    },
    ...
    getBetDataHis(obj){
      ...
      AuthenticationService.getBetsListHistory(queryString)
        .then((resp) => {
    ...
    getTotalBetDataAmount(dataObj){
      var query = '';
      if(dataObj){
        query = Object.keys(dataObj).map(key => `${key}=${dataObj[key]}`).join('&');

      }else{
        query = null
      }
      AuthenticationService.getBetAmount(query).then(res => {
        if(res.data.data){
          if(!res.data.success){
            localStorage.removeItem('token');
            this.$router.push('/login/login').catch(() => {})
          }else{
            this.dataGetAmountBet = res.data.data;
          }
        ...
        console.log(err);
        this.$vs.notify({
          text: 'Lỗi khi lấy dữ liệu số tiền đặt cược!',
          color: 'danger',
          iconPack: 'feather',
          icon: 'icon-heart'
        });
      ...
      //     return this.$vs.notify({
      //       text: 'Nhập email và thời gian hợp lệ để tra soát!',
      //       color: 'danger',
      //       iconPack: 'feather',
      //       icon: 'icon-heart'
      //     });
     ...
     reloadList(){
       ...
       this.$store.dispatch('setToken', token)
       let obj = {
         startDate: this.formatDateSearch(this.startDate),
         endDate: this.formatDateSearch(this.endDate)
       }
       let dataObj = {
         startDate: this.formatDateSearch(this.startDate),
         endDate: this.formatDateSearch(this.endDate),
       ...
       if (this.startDate && this.endDate && this.endDate > this.startDate) {
         // Thực hiện các hành động khi ba biến này đều có giá trị và thỏa mãn điều kiện
         this.getBetDataHis(dataObj)
         this.getTotalBetDataAmount(dataObj)
       } else if (this.startDate && this.endDate && !this.emailSearch) {
         // Thực hiện các hành động khi chỉ có this.startDate và this.endDate có giá trị nhưng không có giá trị cho this.emailSearch
         if(this.startDate > this.endDate){
           return this.$vs.notify({
                   text: 'Thời gian tra soát không hợp lệ!',
                   color: 'danger',
                   iconPack: 'feather',
                   icon: 'icon-heart'
                 });
         }else{
           this.getBetDataHis(obj)
           this.getTotalBetDataAmount(obj)
         }
       } else if (this.emailSearch && !this.startDate && !this.endDate) {
         // Thực hiện các hành động khi chỉ có giá trị cho this.emailSearch mà không có giá trị cho this.startDate và this.endDate
         this.getBetDataHis({
           email:this.emailSearch
         })
         this.getTotalBetDataAmount({
           email:this.emailSearch
         })
       }else if (!this.startDate && !this.endDate && !this.emailSearch) {
         // Thực hiện các hành động khi cả ba biến này đều không có giá trị
         this.getBetDataHis()
         this.getTotalBetDataAmount()
       }
       else {
         return this.$vs.notify({
                 text: 'Nhập thông tin hợp lệ để tra soát!',
                 color: 'danger',
                 iconPack: 'feather',
                 icon: 'icon-heart'
               });
         // Thực hiện các hành động khi ba biến này không thỏa mãn bất kỳ trường hợp nào
       }
...
<style>
.today{
  border: 1px solid #5950b8!important;
}
</style>
