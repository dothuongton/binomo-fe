...
<template>
  <div id="list-trade-member" class="data-list-container">
    ...
    <div id="loading-corners" class="vs-con-loading__container">
      <div class="mb-2">
        <p>
          <span class="mr-4 text-primary" style="color: #ffb0b0"
          >Tổng tiền nạp: <b>{{ formatPrice(amountRecharge) }} VND</b></span
          >

        </p>
        <p>
          <span class="mr-4 text-danger" style="color: #ffb0b0"
          >Tổng tiền rút: <b>{{ formatPrice(amountWithdrawal) }} VND</b></span
          >
        </p>
      ...
      >
        ...
        >
          ...
          >
            ...
            <!-- RELOAD -->
            <div
              class="btn-add-new p-3 mb-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-success border border-solid border-success"
              @click="reloadList"
            >
          ...
          <div class="flex mb-6">
            <div class="flex mt-5">
              <label for="startDatePicker" class="mt-2 mr-1">Từ ngày:</label>
              <datepicker
                id="startDatePicker"
                class=" focus:border-opacity-50"
                v-model="startDate"
              ></datepicker>
              <span class="mt-2 mr-1">-</span>
              <label for="endDatePicker" class="mt-2">Đến ngày:</label>
              <datepicker
                class="ml-5 mr-2 focus:border-opacity-50"
                id="endDatePicker"
                v-model="endDate"
              ></datepicker>
            </div>
            <button class="btn-filter text-base" @click="reloadList()">Tra soát</button>
          </div>
          <div class="mt-2 mr-20">
            <label class="mr-2" for="typeSelect">Type:</label>
            <select class="type-select mb-2" id="typeSelect" v-model="valueType">
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="trade-amount">
                ...
                <span style="color: #29c770" v-else
                >+{{ formatPrice(tr.amount) }}</span
                >
            ...
            <vs-td>
              <p class="trade-note">{{ tr.note ? tr.note.split("/")[1] : "" }}</p>
            </vs-td>
            ...
            <vs-td>
              <p class="trade-create">{{ tr.created_at }}</p>
            </vs-td>
...
export default {
  ...
  data() {
    return {
      ...
      dataValueType: [
        {
          text:"Transfer money from admin -> USDT Wallet",
          value:"Transfer money from admin -> USDT Wallet"
        },
        {
          text:"Buy Vip account",
          value:"Buy Vip account"
        },
        {
          text:"Trừ tiền",
          value: "ttadmin"
      ...
      productsFake: [
        ...
        },
        {
          ...
          to_u: "431344",
          type_key: "rt",
          type: "Money transfer from (Internal) -> Live Account",
          currency: "sys",
          amount: 100.0,
          note: "",
          status: 1,
  ...
  methods: {
    // filterType(){
    //   const queryString = "type=" + this.valueType
    //   AuthenticationService.getTradeListHistory(queryString)
    //     .then((resp) => {
    ...
    reloadList() {
      ...
      AuthenticationService.getTradeListHistoryOfAgency(queryString).then((resp) => {
        this.$vs.loading.close("#loading-corners > .con-vs-loading");
    ...
    getDTNap() {
      AuthenticationService.getRevenueNapOfAgency().then((resp) => {
        let g = resp.data;
        if (g.success == 1) {
          // this.dtUSD = g.data.dtUSD;
          // this.dtBNB = g.data.dtBNB;
          // this.feeBNB = g.data.freeBNB;
          this.amountRecharge = g.data.total
        }
      });
    },

    getDTRut() {
      AuthenticationService.getRevenueRutOfAgency().then((resp) => {
        let g = resp.data;
        if (g.success == 1) {
          // this.dtUSDR = g.data.dtUSD;
          // this.dtBNBR = g.data.dtBNB;
          // this.feeBNBR = g.data.freeBNB;
          this.amountWithdrawal = g.data.total
