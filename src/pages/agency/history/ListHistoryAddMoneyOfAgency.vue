...
<!--          <span class="mr-4" style="color: #00ffad;">Tổng tiền đã thêm PAYPAL: <b>{{ formatPrice(tPAYPAL, 2) }}</b></span>-->
          <span class="mr-4" style="color: #00ffad;">Tổng tiền đã thêm VN: <b>{{ formatPrice(tVN) }}</b></span>
        </p>
      </div>
      <vs-table ref="table" multiple v-model="selectedUser" pagination :max-items="itemsPerPage" search :data="products">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <!-- TRASH -->
            <div class="btn-add-new p-3 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-danger border border-solid border-danger" @click.stop="trashDataDeposit">
              <feather-icon icon="TrashIcon" svgClasses="h-4 w-4" />
              <span class="ml-2 text-base text-danger">Thùng rác</span>
            </div>

            <!-- RELOAD -->
            <div class="btn-add-new p-3 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-success border border-solid border-success" @click="reloadList">
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="de-name font-medium truncate">
                <span v-if="tr.type === 0">Thường</span>
                <span v-if="tr.type === 1">Thường + VIP</span>
                <span v-if="tr.type === 2">Marketing</span>
                <span v-if="tr.type === 3">Marketing + VIP</span>
              </p>
            </vs-td>
            ...
            <vs-td>
              <p class="de-create">{{ tr.title }}</p>
            </vs-td>
            <vs-td>
              <p class="de-create">{{contentAddMoney(tr.content)}}</p>
            </vs-td>
            <vs-td>
...
export default {
  ...
  methods: {
    ...
    reloadList(){
      ...
      AuthenticationService.getAddMoneyListHistoryOfAgency(queryString)
        .then((resp) => {
    ...
    getTotalAddMoney(){
      AuthenticationService.getTotalAddMoneyOfAgency()
        .then((resp) => {
          let g = resp.data;
