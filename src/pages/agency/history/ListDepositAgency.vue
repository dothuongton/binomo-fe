...
import DataViewSidebar from "@/pages/agency/account/slidebar/HandleMoneyNap.vue";
...
export default {
  ...
  methods: {
    ...
    reloadList() {
      ...
      AuthenticationService.getDepositListHistoryOfAgency(result).then((resp) => {
  ...
  created() {
    AuthenticationService.getAllRefOfAgency().then((resp) => {
      ...
      if (resp.data.success == 4) {
        ...
        this.productsFake = resp.data.data.cap1;
      }
    });
    this.reloadList();
    this.getDTNap();
