...
<!--            </vs-dropdown>-->
        ...
        <template slot-scope="{data}">
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="verify-id_front" @click="getPopupViewImg(tr.id_front,tr.nick_name, 'front')" color="primary" type="border"><img width="50" :src="`${mediaType(tr.id_front)}`"></p>
            </vs-td>

            <vs-td>
              <p class="verify-id_back" @click="getPopupViewImg(tr.id_back, tr.nick_name, 'back')"><img width="50" :src="`${mediaType(tr.id_back)}`"></p>
            </vs-td>
    ...
    <!--      <vs-button>Cập nhật</vs-button>-->
    <!--    </vs-popup>-->


    <vs-popup class="holamundo" title="Hình ảnh chi tiết trước" :active.sync="popupViewImgFront">
      <p class="text-center"><img class="responsive" :src="imgLink"></p>
      <input type="file" @change="successUploadFront($event.target.files)">
      <vs-button @click.prevent = updateImgFront>Cập nhật</vs-button>
    </vs-popup>
    <vs-popup class="holamundo" title="Hình ảnh chi tiết sau" :active.sync="popupViewImgBack">
      <p class="text-center"><img class="responsive" :src="imgLink"></p>
      <input type="file" @change="successUploadBack($event.target.files)">
      <vs-button @click.prevent = updateImgBack>Cập nhật</vs-button>
    </vs-popup>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      levelAgency:"cap1",
      dm: config.domain,
      popupViewImgFront:false,
      popupViewImgBack:false,
  ...
  methods: {
    updateImgBack(){
      if(this.forFormBack){
        AuthenticationService.uploadPassportBack(this.forFormBack)
          .then(res=>{
            if(res.data){
              this.getListDataVerify()
              this.popupViewImgBack = false
              this.$vs.notify({
    ...
    updateImgFront(){
      if(this.forFormFront){
        AuthenticationService.uploadPassportFront(this.forFormFront)
          .then(res=>{
            if(res.data){
              this.getListDataVerify()
              this.popupViewImgFront = false
              this.$vs.notify({
                text: 'Thay đổi ảnh thành công!',
                color:'success',
                iconPack: 'feather',
    ...
    successUploadFront(file) {
      ...
      formData.append("nick", this.currentNickname);
      this.forFormFront = formData
    },
    ...
    doneVerify(id, val, index) {
      ...
      AuthenticationService.verifiedUserOfAgency(obj)
        .then((resq) => {
    ...
    getPopupViewImg(link,nickname, type){
      ...
      if(type === 'front'){
        this.popupViewImgFront = true;
      }else{
        this.popupViewImgBack = true
      }
    ...
    getListDataVerify(level){
      AuthenticationService.getAllRefOfAgency()
        .then((resp) => {
  ...
  created() {
    ...
    this.getListDataVerify(this.levelAgency)
