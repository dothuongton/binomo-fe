    ...
    >
      <div class="p-6">
        ...
        <span
          ...
          v-show="errors.has('item-first_name')"
          >{{ errors.first("item-first_name") }}</span
        >
        ...
        <span
          class="text-danger text-sm"
          v-show="errors.has('item-last_name')"
          >{{ errors.first("item-last_name") }}</span
        >
        ...
        <vs-input
          ...
          v-validate=" isRequired ? 'required|min:6|max:16' : '' "
          name="password"
          label="Password"
...
<!--        />-->
        ...
        <vs-textarea
          ...
          icon-pack="feather"
          class="mt-5 w-full"
          v-model="addContentMoneyTransfer"
          v-validate="{ required: true}"
        />
        <div class="flex flex-wrap items-center p-6" slot="footer">
          <vs-button
...
export default {
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        this.addTitleMoneyTransfer = "";
        this.addContentMoneyTransfer = "";
  ...
  data() {
    return {
      ...
      addDataMoneyETH: 0,
      addDataMoneyUSDT: 0,
      addTitleMoneyTransfer:"",
      addContentMoneyTransfer:"",
      typeAccount:0,
      category_choices: [
  ...
  methods: {
    // getTypeAccount() {
    //   AuthenticationService.getInfoUserAdmin(this.dataId)
    //     .then(res => {
    //       const mkt = res.data.data.mkt
    //       const vip = res.data.data.vip
    //       if (mkt === 0) {
    //         if (vip === 0) {
    //             this.typeAccount = 0 //thường
    //         } else if (vip === 1) {
    //             this.typeAccount = 1 //thường + vip
    //         } else {
    //           console.log("Variable 2 is not 0 or 1.");
    //         }
    //       } else if (mkt === 1) {
    //         if (vip === 0) {
    //             this.typeAccount = 2 // marketing
    //         } else if (vip === 1) {
    //             this.typeAccount = 3// marketing+vip
    //         } else {
    //           console.log("Variable 2 is not 0 or 1.");
    //         }
    //       } else {
    //         console.log("Variable 1 is not 0 or 1.");
    //       }
    //     })
    // },
    ...
    submitDataUpdateMoney() {
      const obj = {
        ...
        title:this.addTitleMoneyTransfer,
        content:this.addDataMoneyVN+"/"+this.addContentMoneyTransfer
      };
    ...
    submitDataUpdateUser() {
      this.$validator.validateAll().then((result) => {
        if (result) {
          ...
          // thêm mới tài khoản
          if (Object.entries(this.data).length == 0) {
            AuthenticationService.checkEmailOfAgency(this.dataEmail).then((resp) => {
              if (resp.data.success) {
                return this.$vs.notify({
                ...
                AuthenticationService.createUserOfAgency(obj).then((resp) => {
                  if (resp.data.success) {
            ...
            // cập nhật tài khoản
            AuthenticationService.updateMemberOfAgency(obj).then((resp) => {
              if (resp.data.success) {
                // window.location.reload()
                return this.$vs.notify({
