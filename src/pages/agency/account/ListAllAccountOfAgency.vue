        ...
        <template slot-scope="{ data }">
          ...
          <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
            <vs-td>
              ...
              -- Địa chỉ USDT:
              <span style="color: #26a17b" @click="clickGetAddress(tr.address_USDT, tr.privateKey_USDT, '')
                  ">{{ tr.address_USDT }}</span>
              <br />
              *** Tổng CHƠI:
              <span class="italic" style="color: #ff9900">${{ formatPrice(tr.pricePlay, 2) }}</span><br />
              *** Tổng THẮNG:
              <span class="italic" style="color: #10ff00">${{ formatPrice(tr.priceWin, 2) }}</span><br />
              *** Tổng THUA:
              <span class="italic" style="color: #ff5c5c">${{ formatPrice(tr.priceLose, 2) }}</span><br />
              *** Lời:
              <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.priceWin - tr.priceLose, 2)
                }}</span><br />
              <div v-show="(tr.active_2fa == 1)">
                *** 2FA:
                <span class="italic" style="color: #26a17b">{{ tr.secret_2fa }}</span><br />
              </div>
            </vs-td>
...
<!--                <vs-checkbox v-model="tr.marketing" @click="changeAccType(tr)">Marketing-->
<!--                </vs-checkbox>-->
              </p>
              <p>
            ...
            <vs-td>
              <p class="user-price">
                <img width="20" :src="require('@/assets/images/sky/ic_vnd.png')" />
...
<!--                <br />-->
<!--                ${{ formatPrice(tr.balance_real_account, 2) }}-->
              </p>
            </vs-td>
            ...
            <vs-td>
              <p class="user-create">{{ tr.created_at }}</p>
            </vs-td>

            <vs-td class="whitespace-no-wrap text-center">
              <vx-tooltip v-if="tr.marketing" style="float: left" :title="tr.nick_name" color="success"
                          text="Cộng tiền vào tài khoản">
                <vs-button color="dark" type="line" icon-pack="feather" icon="icon-dollar-sign"
                           @click.stop="addMoneyUser(tr)"></vs-button>
              </vx-tooltip>
              <vx-tooltip style="float: left" :title="tr.nick_name" color="warning" text="Chỉnh sửa tài khoản">
                <vs-button color="dark" type="line" icon-pack="feather" icon="icon-edit"
                           @click.stop="editUser(tr)"></vs-button>
              </vx-tooltip>
              <vx-tooltip style="float: left" :title="tr.nick_name" color="danger" text="Xóa tài khoản">
                <vs-button color="dark" type="line" icon-pack="feather" icon="icon-trash" @click="openPopDelete({
                      id: tr.id,
                      email: tr.email,
                      index: indextr,
                    })
    ...
    <vs-popup background-color="rgba(255,255,255,.6)" class="" title="Background" :active.sync="popupDeleteActive">
      ...
      <vs-button icon="icon-trash" icon-pack="feather" type="gradient"
                 @click.stop="deleteUser(UserInfo.id, UserInfo.index, UserInfo.email)">Đồng ý
      </vs-button>
    </vs-popup>
    <vs-popup title="Address Wallet" :active.sync="popupAdressWallet">
      <p>
        Address: {{ getAdress }}
        <feather-icon icon="CopyIcon" v-clipboard:copy="getAdress" v-clipboard:success="onCopy"
                      class="cursor-pointer"></feather-icon>
        <br />
        Private Key: {{ getPrivateKey }}
        <feather-icon icon="CopyIcon" v-clipboard:copy="getPrivateKey" v-clipboard:success="onCopy"
                      class="cursor-pointer"></feather-icon>
        <br />
        WFI BTC Address: {{ getWfiKey }}
        <feather-icon icon="CopyIcon" v-clipboard:copy="getWfiKey" v-clipboard:success="onCopy"
                      class="cursor-pointer"></feather-icon>
      </p>
    </vs-popup>
    <vs-popup title="Danh sách thành viên F1 - F5" :active.sync="popupF1F7">
...
export default {
  ...
  methods: {
          ...
          for (let obj in data) {
            ...
            for (let i = 0; i < data[obj].length; i++) {
              this.treeData[0].children.push({
                title: `<span style='color: white'>Cấp ${c} - ${data[obj][i].nick_name
    ...
    changeAccType(tr) {
      ...
      AuthenticationService.changeAccMarketingOfAgency(obj).then((resp) => {
    ...
    getListAllAgencyAccount(level) {
      AuthenticationService.getAllRefOfAgency().then((resp) => {
