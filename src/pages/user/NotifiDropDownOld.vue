<template>
  <div style="display: none;">
    ...
    >
      <div class="img-nav old_notification">
        <img
          :src="require('@/assets/images/landing_v2/icon-noti.png')"
          alt=""
        />
      </div>
      <div class="text-item-nav block ttM xl:text-lg">Notifications</div>
        ...
        <div id="loading-corners" class="vs-con-loading__container">
          ...
          >
            <ul class="bordered-items">
              <div
                v-if="unreadOldNotifications.length === 0"
              ...
              <li
                v-for="ntf in unreadOldNotifications"
                :key="ntf.index"
...
<path id="Path_35319" data-name="Path 35319" d="M4.079,8.158A4.091,4.091,0,0,1,0,4.079a.482.482,0,0,1,.51-.51.482.482,0,0,1,.51.51A3.059,3.059,0,1,0,4.079,1.02a.482.482,0,0,1-.51-.51A.482.482,0,0,1,4.079,0a4.079,4.079,0,0,1,0,8.158Z" transform="translate(0 0)" fill="#fcfcfd"></path>
  ...
  mounted() {
    if (!connection) {
      connection = true;
      this.connection = new WebSocket(process.env.BASE_URL_SOCKET_NOTIFY);
      ...
      this.connection.onmessage = function (event) {
        ...
        if (data.typeSocket === "notifiSmsOld") {
          this.countNews += 1;
          this.unreadOldNotifications = [
            {
              index: this.unreadOldNotifications.length,
              title: data.title,
              msg: data.content,
              type: data.type,
              time: moment().fromNow(),
          ...
          ].concat(this.unreadOldNotifications);
        }
      }.bind(this);
