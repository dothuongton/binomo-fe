<template>
  <div @click.stop = handleClickNotify>
    <vs-dropdown
      vs-custom-content
      vs-trigger-click
    ...
    >
      <div class="img-nav">
        <img
          :src="require('@/assets/images/landing_v2/icon-noti.png')"
          alt=""
        />
      </div>
      <div class="text-item-nav block ttM xl:text-lg md:text-base">{{$t("Notifications")}}</div>
      <span class="notification-dropdown-button-number">{{ countNews }}</span>

      <div v-if="$store.state.isOpenNotifyDropDown">
        <vs-dropdown-menu  class="dropdown-custom vx-navbar-dropdown">
          <div class="wrapper-notification">
            ...
            <div class="right" @click="goNotificationPage()">See all</div>
          </div>
              ...
              <ul class="bordered-items">
                ...
                >
                  ...
                  ></span>
                    ...
                    >{{ $t(`${ntf.title }`)}}</span
                    >
                      <small v-if="ntf.type== 'add_money'" >

                        <span class="font-bold">+{{ntf.msg.split("/")[0]}} đ </span>
                        <span>{{ntf.msg.split("/")[1]}}</span>
                        <br/>
                        <span class="font-bold">{{$t('Transfer from')}}</span>: Admin
                        <!--                      {{ntf.msg.split("/")[1]}}-->
                      </small>
                      <small v-else v-html="$t(`${ntf.msg}`)"></small>
...
export default {
  ...
  methods: {
    handleClickNotify(){
      this.$store.commit("IS_OPEN_NOTIFY_DROPDOWN", true);
...
<path id="Path_35319" data-name="Path 35319" d="M4.079,8.158A4.091,4.091,0,0,1,0,4.079a.482.482,0,0,1,.51-.51.482.482,0,0,1,.51.51A3.059,3.059,0,1,0,4.079,1.02a.482.482,0,0,1-.51-.51A.482.482,0,0,1,4.079,0a4.079,4.079,0,0,1,0,8.158Z" transform="translate(0 0)" fill="#fcfcfd"></path>
  ...
  mounted() {
    ...
    if (!connection) {
      connection = true;
      this.connection = new WebSocket(process.env.VUE_APP_BASE_URL_SOCKET_NOTIFY);
      ...
      this.connection.onmessage = function (event) {
        ...
        if (data.typeSocket === "notifiSms") {
          ...
          if (data.type == "add_money") {
            this.isTransferPop = true
            setTimeout(() => {
              this.isTransferPop = false
            }, 5000)
