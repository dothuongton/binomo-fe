<!-- =========================================================================================
  File Name: AddNewDataSidebar.vue
  Description: Add New Data - Sidebar component
  ----------------------------------------------------------------------------------------
  Item Name: Vuexy - Vuejs, HTML & Laravel Admin Dashboard Template
  Author: Ares
  Author URL:
========================================================================================== -->

<template>
  <vs-sidebar
    :hidden-background="!isSideBar"
    click-not-close
    position-right
    :parent="isSideBar ? 'body' : '.c-body'"
    default-index="1"
    color="primary"
    class="rightBar add-new-data-sidebar items-no-padding"
    spacer
    v-model="isSidebarActiveLocal"
    :style="isSideBar ? `position: absolute;` : ''"

  >
    <div class="mt-6 flex items-center justify-between px-6" v-if="isSideBar">
      <feather-icon
        icon="XIcon"
        @click.stop="isSidebarActiveLocal = false"
        class="cursor-pointer"
      ></feather-icon>
    </div>

    <div
    style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px)"
      class="scroll-area--data-list-add-new"
    >
      <div class="p-4 tablist"  id="stopProp">
        <vs-tabs alignment="fixed">
          <vs-tab
            :label="$t('OPEN')"
            @click.stop="(isCloseTab = true), (isOpenTab = false)"
            class="tabClose"
          >
            <div class="con-tab-ejemplo">
              <div
                v-if="listItemOpen.length == 0"
                class="flex flex-col justify-center items-center h-a"
              >
                <img
                  src="data:image/svg+xml;base64,"
                  alt=""
                  class="imageLandScape"
                />
                <p class="text-sm mt-3 color-grey text-center">
                  {{$t("You don't have any open orders yet")}}
                </p>
              </div>
              <div v-else class="flex flex-col justify-center mb-20">
                <VuePerfectScrollbar
              style="height: calc(var(--vh, 1vh) * 100 - 16px - 100px - 10px)"
              class="scroll-area--data-list-add-new"
            >
                <div :key="indextr" v-for="(oknha, indextr) in listItemOpen">
                  <p class="font-16 color-gray mt-3 mb-3">
                    {{ fromatTimzoneD(oknha.create_at) }}
                  </p>
                  <div class="transactionBox text-sm mb-2">
                    <div class="flex justify-between mb-2">
                      <span class="text-sm font-bold text-white"
                        >{{ oknha.currency }}
                        <span class="accType text-xs">{{
                          oknha.type_account == 1 ? "LIVE" : "DEMO"
                        }}</span>
                      </span>
                      <span class="icon iconBitcoin"></span>
                    </div>
                    <div class="flex justify-between mb-2 text-white">
                      <span class="trendGroup flex align-items-center">
                        <span
                          class="trendIcon mr-2"
                          :class="{
                            trendUp: oknha.buy_sell == 'buy',
                            trendDown: oknha.buy_sell == 'sell',
                          }"
                        ></span>
                        <span class="uppercase">{{ $t(`${oknha.buy_sell }`)}}</span>
                      </span>
                      <span>đ{{ formatPrice(oknha.amount_bet) }}</span>
                    </div>
                    <div class="colorGray">{{$t('Session')}}: {{convertSessionOpen(oknha.created_at)}}</div>
                    <div class="flex justify-between bet-time">
                      <span class="colorGray">{{oknha.close}}</span>
                      <span class="text-sm font-bold">{{
                        // convertTimeExpired((oknha.expired_at))
                        convertTimeExpired(oknha.created_at)
                      }}</span>
                    </div>
                  </div>
                </div>
                </VuePerfectScrollbar>
              </div>
            </div>
          </vs-tab>
          <vs-tab
            :label="$t('CLOSE')"
            @click="(isCloseTab = false), (isOpenTab = true)"
            class="tabOpen"
          >
            <div class="con-tab-ejemplo">
              <div
                v-if="listItem.length == 0"
                class="flex flex-col justify-center items-center h-a"
              >
                <img
                  src="data:image/svg+xml;base64,"
                  alt=""
                  class="imageLandScape"
                />
                <p class="text-sm mt-3 color-grey text-center">
                  You don't have any close orders yet
                </p>
              </div>
              <div v-else class="flex flex-col justify-center mb-20">
                <VuePerfectScrollbar
              style="height: calc(var(--vh, 1vh) * 100 - 16px - 100px - 10px)"
              class="scroll-area--data-list-add-new"
            >
                <div :key="indextr" v-for="(oknha, indextr) in listItem">

                  <div class="transactionBox text-sm mb-2">
                    <div class="flex justify-between mb-2">
                      <span class="text-sm font-bold text-white"
                        >{{ oknha.c }}
                        <span class="accType text-xs">{{
                          oknha.t ? "LIVE" : "DEMO"
                        }}</span>
                      </span>
                      <span class="icon iconBitcoin"></span>
                    </div>
                    <div class="flex justify-between mb-2 text-white">
                      <span class="trendGroup flex align-items-center">
                        <span
                          class="trendIcon mr-2"
                          :class="{
                            trendUp: oknha.bs == 'buy',
                            trendDown: oknha.bs == 'sell',
                          }"
                        ></span>
                        <span class="uppercase">{{ $t(`${oknha.bs}`) }}</span>
                      </span>
                      <span>{{ formatPrice(oknha.ab, 2) }}</span>
                    </div>
                    <div>{{oknha.bhm}}</div>
                    <div>{{$t('Close')}}: {{oknha.cl}}</div>
                    <div>{{$t('Session')}}: {{convertSessionOnCloseTab(oknha.d)}}</div>
                    <div class="flex justify-between">
                      <span class="text-xs time">{{
                        formatDay(oknha.d) + " " + formatHours(oknha.d)
                      }}</span>
                      <span
                        class="totalWin font-bold"
                        :class="{ winUp: oknha.aw > 0, winDown: oknha.aw == 0 }"
                      >
                        <span class="uppercase"></span>
                        <span>{{ oknha.aw > 0 ? "+" : "" }}</span
                        >đ{{ oknha.aw > 0 ? formatPrice(oknha.aw, 2) : 0 }}
                      </span>
                    </div>
                  </div>

                </div>
                 </VuePerfectScrollbar>
              </div>
            </div>
          </vs-tab>
        </vs-tabs>
      </div>
    </div>
  </vs-sidebar>
</template>

<script>
import VuePerfectScrollbar from "vue-perfect-scrollbar";
import AuthenticationService from "@/services/AuthenticationService";
import { isTablet, isMobile } from "mobile-device-detect";
import moment from "moment";
import { mapGetters } from "vuex";

export default {
  props: {
    isSidebarActive: {
      type: Boolean,
      required: true
    },
    data: {
      type: Object,
      default: () => {}
    }
  },
  watch: {
    // isSidebarActive(val) {
    //   if(!val) return
    //   if(Object.entries(this.data).length === 0) {
    //     //this.initValues()
    //     this.$validator.reset()
    //   }else {
    //     let { token } = JSON.parse(JSON.stringify(this.data))
    //     this.token = token
    //     //this.initValues()
    //   }
    //   // Object.entries(this.data).length === 0 ? this.initValues() : { this.dataId, this.dataName, this.dataCategory, this.dataOrder_status, this.dataPrice } = JSON.parse(JSON.stringify(this.data))
    // }
  },
  data() {
    return {
      isCloseTab: true,
      isOpenTab: false,

      isDate: "",

      lastDate: "",
      listItem: [],
      listItemOpen: [],
      colorxChangePass: "#def1d1",
      popupActiveChangePass: false,
      // Data Sidebar
      SidebarHSSetting: false,
      sidebarDataSetting: {},
      settings: {
        // perfectscrollbar settings
        maxScrollbarLength: 60,
        wheelSpeed: 0.6
      },
      isSideBar: isMobile || isTablet
    };
  },
  //   filters: {
  //     currencydecimal (val) {
  //       if(val == 'BTC') return val.toFixed(6)
  //       if(val == 'ETH') return val.toFixed(4)
  //       return val.toFixed(2)
  //     }
  //   },
  computed: {
    ...mapGetters("tab", ["getCoinTab"]),
    isSidebarActiveLocal: {
      get() {
        if (this.isCloseTab) {
          this.getListBET(this.getCoinTab.id);
        }
        if (this.isOpenTab) {
          this.getListOrder(this.getCoinTab.id);
        }
        return this.isSidebarActive;
      },
      set(val) {
        if (!val) {
          this.$emit("closeSidebar");
          // this.$validator.reset()
          // this.initValues()
        }
      }
    }
  },
  methods: {
    convertSessionOnCloseTab(value){
      if (value) {
        return moment(value).subtract(1, 'minutes').format('YYYYMMDDHHmm');
      }
    },
    convertTimeExpired(timestamp){
      const date = new Date(timestamp);

      const hour = ('0' + date.getHours()).slice(-2);
      const minute = ('0' + date.getMinutes()).slice(-2);
      const second = ('0' + date.getSeconds()).slice(-2);

      return `${hour}:${minute}:${second}`;
    },
    convertSession(day) {
      const regex = /\d+/g;
      const matches = day.match(regex);
      const result = matches ? matches.join('') : '';
      return result.slice(0, -2);
    },
    convertSessionOpen(timestamp){
      const date = new Date(timestamp);

      const year = date.getFullYear();
      const month = ('0' + (date.getMonth() + 1)).slice(-2);
      const day = ('0' + date.getDate()).slice(-2);
      const hour = ('0' + date.getHours()).slice(-2);
      const minute = ('0' + date.getMinutes()).slice(-2);

      return `${year}${month}${day}${hour}${minute}`;
    },
    fromatTimzoneD(value) {
      if (value) {
        return moment(new Date(value)).format("DD/MM/YYYY");
      }
    },

    fromatTimzoneH(value) {
      if (value) {
        return moment(new Date(value)).format("HH:mm:ss");
      }
    },

    formatPrice(value, minimum) {
      var formatter = new Intl.NumberFormat("en-US", {
        //style: 'currency',
        //currency: '',
        minimumFractionDigits: minimum
      });
      return formatter.format(value);
    },

    formatDay(value) {
      if (value) {
        return moment(String(value)).format("DD/MM/YYYY");
      }
    },

    formatHours(value) {
      // if (value) {
      //   return moment(String(value)).format("HH:mm:ss");
      // }
      if (value) {
        return moment(value).subtract(1, 'minutes').format('HH:mm:ss');
      }
    },

    getListBET(type) {
      AuthenticationService.getListHistoryOrderNew(type).then(res => {
        if (res.data.success) {
          this.listItemOpen = res.data.data;
        }
      });
    },

    getListOrder(type) {
      AuthenticationService.getListHitoryOrder(type).then(res => {
        if (res.data.success) {
          this.listItem = res.data.data;
        }
      });
    },

    showHoSoSetting() {
      //let id = data
      this.toggleDataSidebar(true);
    },

    toggleDataSidebar(val = false) {
      this.SidebarHSSetting = val;
    }
  },
  components: {
    VuePerfectScrollbar
  },

  mounted() {
    let stateOpen = localStorage.getItem("stateOpen");
    if (stateOpen) {
      const list = JSON.parse(stateOpen);
      if (list) {
        this.isCloseTab
          ? setTimeout(() => {
              this.getListBET(this.getCoinTab.id);
            }, 800)
          : setTimeout(() => {
              this.getListOrder(this.getCoinTab.id);
            }, 800);
      }
    }

    window.addEventListener("stateOpen-localstorage-changed", event => {
      if (!event.detail.storage) {
        this.isCloseTab
          ? setTimeout(() => {
              this.getListBET(this.getCoinTab.id);
            }, 800)
          : setTimeout(() => {
              this.getListOrder(this.getCoinTab.id);
            }, 800);
        return;
      }
      const list = JSON.parse(event.detail.storage);
      if (list) {
        this.isCloseTab
          ? setTimeout(() => {
              this.getListBET(this.getCoinTab.id);
            }, 800)
          : setTimeout(() => {
              this.getListOrder(this.getCoinTab.id);
            }, 800);
      }
    });
    // $("#stopProp").on("scroll",function (e) {
    //   e.stopPropagation();
    //   e.preventDefault();
    // })
  }
};
</script>

<style>
.tablist .con-vs-tabs {
  height: calc(var(--vh, 1vh) * 100 - 16px - 75px - 10px);
}

.tablist .con-vs-tabs .con-slot-tabs
/* .tablist .con-vs-tabs .con-slot-tabs .vs-tabs--content,
.tablist .con-vs-tabs .con-slot-tabs .con-tab-ejemplo,
.tablist .con-vs-tabs .con-slot-tabs .con-tab-ejemplo>div */
 {
  /* height: calc(var(--vh, 1vh) * 100 - 16px - 100px - 10px); */
  /* overflow-y: scroll; */
  touch-action: auto;
  -ms-touch-action: auto;
  -webkit-overflow-scrolling: touch;
  width: 100%;
  /* position: absolute;
  top: 0px;
  left: 0px;
  bottom: 0px; */
}

/* .tablist {
  overflow-y: scroll;
  touch-action: auto;
  -ms-touch-action: auto;
  -webkit-overflow-scrolling: touch;
} */
</style>

<style scoped>
.con-tab-ejemplo {
  overflow-y: scroll;
}
.transactionBox {
  padding: 0.5rem;
  border-radius: 6px;
  background-color: rgba(255, 255, 255, 0.14);
}

.transactionBox .accType {
  background-color: #2177ff;
  border-radius: 5px;
  font-size: 14px;
  padding: 3px 5px;
  line-height: 20px;
  vertical-align: middle;
}

.transactionBox .accType.live {
  background-color: #ea5455;
}

.transactionBox .icon {
  display: inline-block;
  width: 19px;
  height: 18px;
  background-repeat: no-repeat;
  background-size: contain;
}

.transactionBox .icon.iconBitcoin {
  background-image: url(data:image/svg+xml;base64,);
}

.transactionBox .trendGroup .trendIcon {
  display: inline-block;
  width: 19px;
  height: 19px;
  background-repeat: no-repeat;
  background-size: contain;
}

.transactionBox .trendGroup .trendIcon.trendUp,
.transactionBox .trendGroup .trendIcon.UP {
  background-image: url(data:image/svg+xml;base64,);
}

.transactionBox .trendGroup .trendIcon.trendDown,
.transactionBox .trendGroup .trendIcon.DOWN {
  background-image: url(data:image/svg+xml;base64,);
}

.transactionBox .time {
  color: rgba(255, 255, 255, 0.5);
}

.transactionBox .totalWin.winUp {
  color: #23c15f;
}

.transactionBox .totalWin.winDown {
  color: #ea5455;
}

.h-a {
  height: calc(100vh - 160px);
}
</style>

<style lang="scss" scoped>
.pro-c-avatar__content {
  align-items: center;
  display: flex;
  height: 100%;
  justify-content: center;
  overflow: hidden;
  position: relative;
  width: 100%;
}
.pro-c-avatar_size_extra-large .pro-c-avatar__placeholder {
  padding: 1rem;
}
.pro-c-avatar__placeholder {
  fill: currentColor;
  box-sizing: border-box;
  height: 100%;
  padding: 0.5rem;
  width: 100%;
}
.vs-sidebar--background {
  background: rgba(0, 0, 0, 0.5);
}
.add-new-data-sidebar {
  ::v-deep .vs-sidebar--background {
    z-index: 52010;
  }

  ::v-deep .vs-sidebar {
    z-index: 52010;
    width: 230px !important;
    max-width: 90vw !important;
    top: 77px;
    @media only screen and(min-width:864px) {
      margin-top: 3rem!important;
    }

    .img-upload {
      margin-top: 2rem;

      .con-img-upload {
        padding: 0;
      }

      .con-input-upload {
        width: 100%;
        margin: 0;
      }
    }
  }
}
@media only screen and(max-width:768px) {
  .add-new-data-sidebar {
    ::v-deep .vs-sidebar--background {
      z-index: 52010;
    }

    ::v-deep .vs-sidebar {
      z-index: 52010;
      width: 100% !important;
      max-width: 100vw !important;
      top: 0px;
    }
  }
}
.scroll-area--data-list-add-new {
  //  height: calc(var(--vh, 1vh) * 100 - 4.3rem);
  //  height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 82px);
}
</style>
